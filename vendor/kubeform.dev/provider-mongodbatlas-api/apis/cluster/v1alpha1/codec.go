/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by Kubeform. DO NOT EDIT.

package v1alpha1

import (
	"unsafe"

	jsoniter "github.com/json-iterator/go"
	"github.com/modern-go/reflect2"
)

func GetEncoder() map[string]jsoniter.ValEncoder {
	return map[string]jsoniter.ValEncoder{
		jsoniter.MustGetKind(reflect2.TypeOf(ClusterSpecAdvancedConfiguration{}).Type1()): ClusterSpecAdvancedConfigurationCodec{},
		jsoniter.MustGetKind(reflect2.TypeOf(ClusterSpecBiConnectorConfig{}).Type1()):     ClusterSpecBiConnectorConfigCodec{},
	}
}

func GetDecoder() map[string]jsoniter.ValDecoder {
	return map[string]jsoniter.ValDecoder{
		jsoniter.MustGetKind(reflect2.TypeOf(ClusterSpecAdvancedConfiguration{}).Type1()): ClusterSpecAdvancedConfigurationCodec{},
		jsoniter.MustGetKind(reflect2.TypeOf(ClusterSpecBiConnectorConfig{}).Type1()):     ClusterSpecBiConnectorConfigCodec{},
	}
}

func getEncodersWithout(typ string) map[string]jsoniter.ValEncoder {
	origMap := GetEncoder()
	delete(origMap, typ)
	return origMap
}

func getDecodersWithout(typ string) map[string]jsoniter.ValDecoder {
	origMap := GetDecoder()
	delete(origMap, typ)
	return origMap
}

// +k8s:deepcopy-gen=false
type ClusterSpecAdvancedConfigurationCodec struct {
}

func (ClusterSpecAdvancedConfigurationCodec) IsEmpty(ptr unsafe.Pointer) bool {
	return (*ClusterSpecAdvancedConfiguration)(ptr) == nil
}

func (ClusterSpecAdvancedConfigurationCodec) Encode(ptr unsafe.Pointer, stream *jsoniter.Stream) {
	obj := (*ClusterSpecAdvancedConfiguration)(ptr)
	var objs []ClusterSpecAdvancedConfiguration
	if obj != nil {
		objs = []ClusterSpecAdvancedConfiguration{*obj}
	}

	jsonit := jsoniter.Config{
		EscapeHTML:             true,
		SortMapKeys:            true,
		ValidateJsonRawMessage: true,
		TagKey:                 "tf",
		TypeEncoders:           getEncodersWithout(jsoniter.MustGetKind(reflect2.TypeOf(ClusterSpecAdvancedConfiguration{}).Type1())),
	}.Froze()

	byt, _ := jsonit.Marshal(objs)

	stream.Write(byt)
}

func (ClusterSpecAdvancedConfigurationCodec) Decode(ptr unsafe.Pointer, iter *jsoniter.Iterator) {
	switch iter.WhatIsNext() {
	case jsoniter.NilValue:
		iter.Skip()
		*(*ClusterSpecAdvancedConfiguration)(ptr) = ClusterSpecAdvancedConfiguration{}
		return
	case jsoniter.ArrayValue:
		objsByte := iter.SkipAndReturnBytes()
		if len(objsByte) > 0 {
			var objs []ClusterSpecAdvancedConfiguration

			jsonit := jsoniter.Config{
				EscapeHTML:             true,
				SortMapKeys:            true,
				ValidateJsonRawMessage: true,
				TagKey:                 "tf",
				TypeDecoders:           getDecodersWithout(jsoniter.MustGetKind(reflect2.TypeOf(ClusterSpecAdvancedConfiguration{}).Type1())),
			}.Froze()
			jsonit.Unmarshal(objsByte, &objs)

			if len(objs) > 0 {
				*(*ClusterSpecAdvancedConfiguration)(ptr) = objs[0]
			} else {
				*(*ClusterSpecAdvancedConfiguration)(ptr) = ClusterSpecAdvancedConfiguration{}
			}
		} else {
			*(*ClusterSpecAdvancedConfiguration)(ptr) = ClusterSpecAdvancedConfiguration{}
		}
	case jsoniter.ObjectValue:
		objByte := iter.SkipAndReturnBytes()
		if len(objByte) > 0 {
			var obj ClusterSpecAdvancedConfiguration

			jsonit := jsoniter.Config{
				EscapeHTML:             true,
				SortMapKeys:            true,
				ValidateJsonRawMessage: true,
				TagKey:                 "tf",
				TypeDecoders:           getDecodersWithout(jsoniter.MustGetKind(reflect2.TypeOf(ClusterSpecAdvancedConfiguration{}).Type1())),
			}.Froze()
			jsonit.Unmarshal(objByte, &obj)

			*(*ClusterSpecAdvancedConfiguration)(ptr) = obj
		} else {
			*(*ClusterSpecAdvancedConfiguration)(ptr) = ClusterSpecAdvancedConfiguration{}
		}
	default:
		iter.ReportError("decode ClusterSpecAdvancedConfiguration", "unexpected JSON type")
	}
}

// +k8s:deepcopy-gen=false
type ClusterSpecBiConnectorConfigCodec struct {
}

func (ClusterSpecBiConnectorConfigCodec) IsEmpty(ptr unsafe.Pointer) bool {
	return (*ClusterSpecBiConnectorConfig)(ptr) == nil
}

func (ClusterSpecBiConnectorConfigCodec) Encode(ptr unsafe.Pointer, stream *jsoniter.Stream) {
	obj := (*ClusterSpecBiConnectorConfig)(ptr)
	var objs []ClusterSpecBiConnectorConfig
	if obj != nil {
		objs = []ClusterSpecBiConnectorConfig{*obj}
	}

	jsonit := jsoniter.Config{
		EscapeHTML:             true,
		SortMapKeys:            true,
		ValidateJsonRawMessage: true,
		TagKey:                 "tf",
		TypeEncoders:           getEncodersWithout(jsoniter.MustGetKind(reflect2.TypeOf(ClusterSpecBiConnectorConfig{}).Type1())),
	}.Froze()

	byt, _ := jsonit.Marshal(objs)

	stream.Write(byt)
}

func (ClusterSpecBiConnectorConfigCodec) Decode(ptr unsafe.Pointer, iter *jsoniter.Iterator) {
	switch iter.WhatIsNext() {
	case jsoniter.NilValue:
		iter.Skip()
		*(*ClusterSpecBiConnectorConfig)(ptr) = ClusterSpecBiConnectorConfig{}
		return
	case jsoniter.ArrayValue:
		objsByte := iter.SkipAndReturnBytes()
		if len(objsByte) > 0 {
			var objs []ClusterSpecBiConnectorConfig

			jsonit := jsoniter.Config{
				EscapeHTML:             true,
				SortMapKeys:            true,
				ValidateJsonRawMessage: true,
				TagKey:                 "tf",
				TypeDecoders:           getDecodersWithout(jsoniter.MustGetKind(reflect2.TypeOf(ClusterSpecBiConnectorConfig{}).Type1())),
			}.Froze()
			jsonit.Unmarshal(objsByte, &objs)

			if len(objs) > 0 {
				*(*ClusterSpecBiConnectorConfig)(ptr) = objs[0]
			} else {
				*(*ClusterSpecBiConnectorConfig)(ptr) = ClusterSpecBiConnectorConfig{}
			}
		} else {
			*(*ClusterSpecBiConnectorConfig)(ptr) = ClusterSpecBiConnectorConfig{}
		}
	case jsoniter.ObjectValue:
		objByte := iter.SkipAndReturnBytes()
		if len(objByte) > 0 {
			var obj ClusterSpecBiConnectorConfig

			jsonit := jsoniter.Config{
				EscapeHTML:             true,
				SortMapKeys:            true,
				ValidateJsonRawMessage: true,
				TagKey:                 "tf",
				TypeDecoders:           getDecodersWithout(jsoniter.MustGetKind(reflect2.TypeOf(ClusterSpecBiConnectorConfig{}).Type1())),
			}.Froze()
			jsonit.Unmarshal(objByte, &obj)

			*(*ClusterSpecBiConnectorConfig)(ptr) = obj
		} else {
			*(*ClusterSpecBiConnectorConfig)(ptr) = ClusterSpecBiConnectorConfig{}
		}
	default:
		iter.ReportError("decode ClusterSpecBiConnectorConfig", "unexpected JSON type")
	}
}
