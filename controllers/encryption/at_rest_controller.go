...
// AtRestReconciler reconciles a AtRest object
type AtRestReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=encryption.mongodbatlas.kubeform.com,resources=atrests,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=encryption.mongodbatlas.kubeform.com,resources=atrests/status,verbs=get;update;patch

func (r *AtRestReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("atrest", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AtRest")
...
func (r *AtRestReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &encryptionv1alpha1.AtRest{}); err != nil {
			klog.Error(err, "unable to set up auditor", encryptionv1alpha1.AtRest{}.APIVersion, encryptionv1alpha1.AtRest{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&encryptionv1alpha1.AtRest{}).
...
func (r *AtRestReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for AtRest")
			return nil
		}
		...
		resourceList := &encryptionv1alpha1.AtRestList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list AtRest")
			return nil
		}
