...
// InvitationReconciler reconciles a Invitation object
...
// +kubebuilder:rbac:groups=project.mongodbatlas.kubeform.com,resources=invitations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=project.mongodbatlas.kubeform.com,resources=invitations/status,verbs=get;update;patch

func (r *InvitationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("invitation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Invitation")
...
func (r *InvitationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &projectv1alpha1.Invitation{}); err != nil {
			klog.Error(err, "unable to set up auditor", projectv1alpha1.Invitation{}.APIVersion, projectv1alpha1.Invitation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&projectv1alpha1.Invitation{}).
