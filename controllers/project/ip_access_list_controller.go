...
// IpAccessListReconciler reconciles a IpAccessList object
type IpAccessListReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=project.mongodbatlas.kubeform.com,resources=ipaccesslists,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=project.mongodbatlas.kubeform.com,resources=ipaccesslists/status,verbs=get;update;patch

func (r *IpAccessListReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ipaccesslist", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IpAccessList")
...
func (r *IpAccessListReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &projectv1alpha1.IpAccessList{}); err != nil {
			klog.Error(err, "unable to set up auditor", projectv1alpha1.IpAccessList{}.APIVersion, projectv1alpha1.IpAccessList{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&projectv1alpha1.IpAccessList{}).
