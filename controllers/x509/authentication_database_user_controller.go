...
// AuthenticationDatabaseUserReconciler reconciles a AuthenticationDatabaseUser object
type AuthenticationDatabaseUserReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=x509.mongodbatlas.kubeform.com,resources=authenticationdatabaseusers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=x509.mongodbatlas.kubeform.com,resources=authenticationdatabaseusers/status,verbs=get;update;patch

func (r *AuthenticationDatabaseUserReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("authenticationdatabaseuser", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AuthenticationDatabaseUser")
...
func (r *AuthenticationDatabaseUserReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &x509v1alpha1.AuthenticationDatabaseUser{}); err != nil {
			klog.Error(err, "unable to set up auditor", x509v1alpha1.AuthenticationDatabaseUser{}.APIVersion, x509v1alpha1.AuthenticationDatabaseUser{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&x509v1alpha1.AuthenticationDatabaseUser{}).
...
func (r *AuthenticationDatabaseUserReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for AuthenticationDatabaseUser")
			return nil
		}
		...
		resourceList := &x509v1alpha1.AuthenticationDatabaseUserList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list AuthenticationDatabaseUser")
			return nil
		}
