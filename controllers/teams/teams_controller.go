...
// TeamsReconciler reconciles a Teams object
type TeamsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=teams.mongodbatlas.kubeform.com,resources=teams,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=teams.mongodbatlas.kubeform.com,resources=teams/status,verbs=get;update;patch

func (r *TeamsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("teams", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Teams")
...
func (r *TeamsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &teamsv1alpha1.Teams{}); err != nil {
			klog.Error(err, "unable to set up auditor", teamsv1alpha1.Teams{}.APIVersion, teamsv1alpha1.Teams{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&teamsv1alpha1.Teams{}).
