...
// PartyIntegrationReconciler reconciles a PartyIntegration object
type PartyIntegrationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=third.mongodbatlas.kubeform.com,resources=partyintegrations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=third.mongodbatlas.kubeform.com,resources=partyintegrations/status,verbs=get;update;patch

func (r *PartyIntegrationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("partyintegration", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PartyIntegration")
...
func (r *PartyIntegrationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &thirdv1alpha1.PartyIntegration{}); err != nil {
			klog.Error(err, "unable to set up auditor", thirdv1alpha1.PartyIntegration{}.APIVersion, thirdv1alpha1.PartyIntegration{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&thirdv1alpha1.PartyIntegration{}).
...
func (r *PartyIntegrationReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for PartyIntegration")
			return nil
		}
		...
		resourceList := &thirdv1alpha1.PartyIntegrationList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list PartyIntegration")
			return nil
		}
