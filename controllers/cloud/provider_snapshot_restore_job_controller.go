...
// ProviderSnapshotRestoreJobReconciler reconciles a ProviderSnapshotRestoreJob object
type ProviderSnapshotRestoreJobReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.mongodbatlas.kubeform.com,resources=providersnapshotrestorejobs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.mongodbatlas.kubeform.com,resources=providersnapshotrestorejobs/status,verbs=get;update;patch

func (r *ProviderSnapshotRestoreJobReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("providersnapshotrestorejob", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProviderSnapshotRestoreJob")
...
func (r *ProviderSnapshotRestoreJobReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.ProviderSnapshotRestoreJob{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.ProviderSnapshotRestoreJob{}.APIVersion, cloudv1alpha1.ProviderSnapshotRestoreJob{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.ProviderSnapshotRestoreJob{}).
