...
// ProviderAccessSetupReconciler reconciles a ProviderAccessSetup object
type ProviderAccessSetupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.mongodbatlas.kubeform.com,resources=provideraccesssetups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.mongodbatlas.kubeform.com,resources=provideraccesssetups/status,verbs=get;update;patch

func (r *ProviderAccessSetupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("provideraccesssetup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProviderAccessSetup")
...
func (r *ProviderAccessSetupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.ProviderAccessSetup{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.ProviderAccessSetup{}.APIVersion, cloudv1alpha1.ProviderAccessSetup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.ProviderAccessSetup{}).
