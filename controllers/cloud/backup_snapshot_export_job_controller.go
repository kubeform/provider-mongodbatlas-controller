...
// BackupSnapshotExportJobReconciler reconciles a BackupSnapshotExportJob object
type BackupSnapshotExportJobReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.mongodbatlas.kubeform.com,resources=backupsnapshotexportjobs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.mongodbatlas.kubeform.com,resources=backupsnapshotexportjobs/status,verbs=get;update;patch

func (r *BackupSnapshotExportJobReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("backupsnapshotexportjob", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BackupSnapshotExportJob")
...
func (r *BackupSnapshotExportJobReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.BackupSnapshotExportJob{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.BackupSnapshotExportJob{}.APIVersion, cloudv1alpha1.BackupSnapshotExportJob{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.BackupSnapshotExportJob{}).
