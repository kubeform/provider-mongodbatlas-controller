...
// ProviderSnapshotReconciler reconciles a ProviderSnapshot object
type ProviderSnapshotReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.mongodbatlas.kubeform.com,resources=providersnapshots,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.mongodbatlas.kubeform.com,resources=providersnapshots/status,verbs=get;update;patch

func (r *ProviderSnapshotReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("providersnapshot", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProviderSnapshot")
...
func (r *ProviderSnapshotReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.ProviderSnapshot{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.ProviderSnapshot{}.APIVersion, cloudv1alpha1.ProviderSnapshot{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.ProviderSnapshot{}).
