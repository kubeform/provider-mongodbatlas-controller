...
// ProviderAccessAuthorizationReconciler reconciles a ProviderAccessAuthorization object
type ProviderAccessAuthorizationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.mongodbatlas.kubeform.com,resources=provideraccessauthorizations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.mongodbatlas.kubeform.com,resources=provideraccessauthorizations/status,verbs=get;update;patch

func (r *ProviderAccessAuthorizationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("provideraccessauthorization", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProviderAccessAuthorization")
...
func (r *ProviderAccessAuthorizationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.ProviderAccessAuthorization{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.ProviderAccessAuthorization{}.APIVersion, cloudv1alpha1.ProviderAccessAuthorization{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.ProviderAccessAuthorization{}).
