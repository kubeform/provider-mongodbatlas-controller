...
// EndpointServiceAdlReconciler reconciles a EndpointServiceAdl object
type EndpointServiceAdlReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=privatelink.mongodbatlas.kubeform.com,resources=endpointserviceadls,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=privatelink.mongodbatlas.kubeform.com,resources=endpointserviceadls/status,verbs=get;update;patch

func (r *EndpointServiceAdlReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("endpointserviceadl", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EndpointServiceAdl")
...
func (r *EndpointServiceAdlReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &privatelinkv1alpha1.EndpointServiceAdl{}); err != nil {
			klog.Error(err, "unable to set up auditor", privatelinkv1alpha1.EndpointServiceAdl{}.APIVersion, privatelinkv1alpha1.EndpointServiceAdl{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&privatelinkv1alpha1.EndpointServiceAdl{}).
