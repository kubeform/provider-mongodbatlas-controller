...
// +kubebuilder:rbac:groups=global.mongodbatlas.kubeform.com,resources=clusterconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=global.mongodbatlas.kubeform.com,resources=clusterconfigs/status,verbs=get;update;patch

func (r *ClusterConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("clusterconfig", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ClusterConfig")
...
func (r *ClusterConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &globalv1alpha1.ClusterConfig{}); err != nil {
			klog.Error(err, "unable to set up auditor", globalv1alpha1.ClusterConfig{}.APIVersion, globalv1alpha1.ClusterConfig{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&globalv1alpha1.ClusterConfig{}).
