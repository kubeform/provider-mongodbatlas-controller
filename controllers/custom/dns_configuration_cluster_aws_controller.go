...
// DnsConfigurationClusterAwsReconciler reconciles a DnsConfigurationClusterAws object
type DnsConfigurationClusterAwsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=custom.mongodbatlas.kubeform.com,resources=dnsconfigurationclusteraws,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=custom.mongodbatlas.kubeform.com,resources=dnsconfigurationclusteraws/status,verbs=get;update;patch

func (r *DnsConfigurationClusterAwsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dnsconfigurationclusteraws", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DnsConfigurationClusterAws")
...
func (r *DnsConfigurationClusterAwsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &customv1alpha1.DnsConfigurationClusterAws{}); err != nil {
			klog.Error(err, "unable to set up auditor", customv1alpha1.DnsConfigurationClusterAws{}.APIVersion, customv1alpha1.DnsConfigurationClusterAws{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&customv1alpha1.DnsConfigurationClusterAws{}).
