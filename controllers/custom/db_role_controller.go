...
// DbRoleReconciler reconciles a DbRole object
type DbRoleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=custom.mongodbatlas.kubeform.com,resources=dbroles,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=custom.mongodbatlas.kubeform.com,resources=dbroles/status,verbs=get;update;patch

func (r *DbRoleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dbrole", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DbRole")
...
func (r *DbRoleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &customv1alpha1.DbRole{}); err != nil {
			klog.Error(err, "unable to set up auditor", customv1alpha1.DbRole{}.APIVersion, customv1alpha1.DbRole{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&customv1alpha1.DbRole{}).
