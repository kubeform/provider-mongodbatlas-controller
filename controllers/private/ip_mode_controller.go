...
// IpModeReconciler reconciles a IpMode object
type IpModeReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=private.mongodbatlas.kubeform.com,resources=ipmodes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=private.mongodbatlas.kubeform.com,resources=ipmodes/status,verbs=get;update;patch

func (r *IpModeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ipmode", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IpMode")
...
func (r *IpModeReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &privatev1alpha1.IpMode{}); err != nil {
			klog.Error(err, "unable to set up auditor", privatev1alpha1.IpMode{}.APIVersion, privatev1alpha1.IpMode{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&privatev1alpha1.IpMode{}).
