...
// AuditingReconciler reconciles a Auditing object
type AuditingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=auditing.mongodbatlas.kubeform.com,resources=auditings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=auditing.mongodbatlas.kubeform.com,resources=auditings/status,verbs=get;update;patch

func (r *AuditingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("auditing", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Auditing")
...
func (r *AuditingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &auditingv1alpha1.Auditing{}); err != nil {
			klog.Error(err, "unable to set up auditor", auditingv1alpha1.Auditing{}.APIVersion, auditingv1alpha1.Auditing{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&auditingv1alpha1.Auditing{}).
